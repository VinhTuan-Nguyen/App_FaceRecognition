...
namespace App_DDSV
{
    public partial class frm01_Qt_StudentEdit : Form
    {
        ...
        public frm01_Qt_StudentEdit(DataGridViewRow row, string mahp)
        {
            if(mahp == "")
            {
                InitializeComponent();
        ...
        private void btn_Delete_Click(object sender, EventArgs e)
        {
            DialogResult result = MessageBox.Show("Xác Nhận Xóa Sinh Viên Này !\n" +
                "Chú ý: Tất Cả Dữ Liệu Của Sinh Viên Này Sẽ Bị Xóa Khỏi CSDL Vĩnh Viễn\nHãy Cân Nhắc",
                    ...
                    try
                    {
                        ...
                        if (result == DialogResult.OK)
                        {
                            ...
                            using (conSql.conn = new SqlConnection(conSql.conString))
                            {
                                ...
                                {
                                    MessageBox.Show("Sinh Viên Này Đã Được Thêm Vào CSDL");
                                }
                                conSql.conn.Close();
                            }
                        }
        ...
        {
            DialogResult result = MessageBox.Show("Xác Nhận Xóa Sinh Viên Khỏi Học Phần Này",
                "Xác Nhận", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
            if (result == DialogResult.OK)
            {
                conSql.query = "DELETE FROM tab_DKHP WHERE col_MaSV = @masv and col_MaHP=@mahp " +
                    "DELETE FROM tab_DiemDanh WHERE col_MaSV = @masv and col_MaHP=@mahp";
                using (conSql.conn = new SqlConnection(conSql.conString))
                {
                    conSql.conn.Open();
                    conSql.cmd = new SqlCommand(conSql.query, conSql.conn);
        ...
        {
            btn_Remove.ForeColor = Color.OrangeRed;
        }

        private void btn_Remove_MouseLeave(object sender, EventArgs e)
        {
            btn_Remove.ForeColor = Color.Brown;
        }
    }
}
