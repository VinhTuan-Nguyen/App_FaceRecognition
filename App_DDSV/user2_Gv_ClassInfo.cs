...
namespace App_DDSV
{
    public partial class user2_Gv_ClassInfo : UserControl
    {
        ...
        {
            frm01_Qt_ClassEdit f = new frm01_Qt_ClassEdit(lb_MaGV.Text);
            f.ShowDialog();
        }

        private void btn_addStudent_Click(object sender, EventArgs e)
        {
            frm02_Gv_AddStudent f = new frm02_Gv_AddStudent(lb_MaGV.Text);
            f.ShowDialog();
        }
        ...
        private void dgv_LoadData_RowHeaderMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                DataGridViewRow row = dgv_LoadData.Rows[e.RowIndex];
                frm01_Qt_StudentEdit f = new frm01_Qt_StudentEdit(row, txt_MaHP.Text);
                f.ShowDialog();
            }
        }

        private void dgv_LoadData_CellContentDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                DataGridViewRow row = dgv_LoadData.Rows[e.RowIndex];
