...
namespace App_DDSV
{
    public partial class frm01_Qt_ClassEdit : Form
    {
        ...
        public frm01_Qt_ClassEdit(string magv)
        {
            InitializeComponent();
            txt_NienKhoa.Enabled = true;
            txt_MaHP.Enabled = true;
                ...
                else
                {
                    ...
                    if(result == DialogResult.OK)
                    {
                        using(conSql.conn = new SqlConnection(conSql.conString))
                        {
                            ...
                            {
                                DialogResult result1 = MessageBox.Show("Mã Học Phần Này Đã Tồn Tại\n" +
                                    "Bạn Có Muốn Chỉnh Sửa Không ?", "Cảnh Báo",
                                    MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
                                if(result1 == DialogResult.OK)
                                {
                                    MessageBox.Show("Vui Lòng Nhấn Nút Save Một Lần Nữa Để Chỉnh Sửa", "Xác Nhận",
                                        MessageBoxButtons.OK, MessageBoxIcon.Information);
                                    txt_MaHP.Enabled = false;
                                }
                            }
                            else
                            {
